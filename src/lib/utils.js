...
export function prepareData(obj){
  ...
  for(const key in obj){
    if(key.startsWith(senderAddressKey)){
      senderAddress[key.replace(senderAddressKey, "")] = obj[key.replace(senderAddressKey, "")]
    }
    if(key.startsWith(clientAddressKey)){
      clientAddress[key.replace(clientAddressKey, "")] = obj[key.replace(clientAddressKey, "")]
    }
